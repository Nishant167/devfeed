...
Maintained as configuration, not hard-coded into application logic, so the list can be extended without a code change. Matching is **whole-token and positional, not substring** — the repository name is split on `-` and `_`, and a pattern only matches the **first or last** token(s) of the name, never a token sitting in the middle. A pattern with a trailing `-` (`awesome-`, `interview-`, `learning-`, `study-`, `practice-`) anchors to the first token(s) only; a pattern with a leading `-` (`-awesome`, `-questions`, `-notes`) anchors to the last token(s) only; a bare pattern with no dash (`tutorial`, `course`, `resources`, `my-portfolio`, `100-days`, etc.) matches at either the first or last position, but never the middle. This is why `cloud-resources-manager` does not match `resources` (it's the middle token, not the first or last) while `awesome-python` does match `awesome-` (it's the first token) — whole-token matching alone isn't sufficient to get that distinction right; position matters too. A match is a **hard exclude**: the repository never enters the candidate set, rather than being folded into the quality score as a penalty — filtering happens before ranking so ranking only ever sees repositories worth ranking (see the quality score below).
